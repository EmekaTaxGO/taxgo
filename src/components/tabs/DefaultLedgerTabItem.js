...
class DefaultLedgerTabItem extends Component {
    ...
    imageRadius = 30;
    imageUrl = 'https://images.unsplash.com/photo-1506784983877-45594efa4cbe?ixid=MXwxMjA3fDB8MHxzZWFyY2h8Mnx8bGVkZ2VyfGVufDB8fDB8&ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=60';
    ...
    listData = () => {
        if (isEmpty(this.state.query)) {
            return this.props.ledger.defaultLedgers;
        } else {
    ...
    filteredLedgers = () => {
        let filteredLedgers = [];
        filteredLedgers = this.props.ledger.defaultLedgers.filter(value => {
    ...
    onEditClick = (item) => {
        this.props.navigation.push('AddLedgerScreen', {
            ledger: item,
            onLedgerUpdated: this.fetchDefaultLedger
        });
    }
    ...
    render() {
        ...
        if (ledger.fetchingDefaultLedger) {
            return <OnScreenSpinner />
        }
        if (ledger.fetchDefaultLedgerError) {
            return <FullScreenError tryAgainClick={this.fetchDefaultLedger} />
        }
        if (ledger.defaultLedgers.length === 0) {
