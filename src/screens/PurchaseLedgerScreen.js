...
class PurchaseLedgerScreen extends Component {
    ...
    componentDidMount() {
        this.fetchPurchaseLedger();
    }

    fetchPurchaseLedger = () => {
        const { ledgerActions } = this.props;
        ledgerActions.getPurchaseLedger();
    }
    ...
    listData = () => {
        if (isEmpty(this.state.query)) {
            return this.props.ledger.purchaseLedgers;
        } else {
    ...
    render() {
        ...
        if (ledger.fetchingPurchaseLedger) {
            return <OnScreenSpinner />
        }
        if (ledger.fetchPurchaseLedgerError) {
            return <FullScreenError tryAgainClick={this.fetchPurchaseLedger} />
        }
        if (ledger.purchaseLedgers.length === 0) {
            return <EmptyView message='No Purchase Ledger available' iconName='description' />
        }
